...
if __name__ == "__main__":
	...
	aparser.add_argument("--ip", "-c",
		help="Specify the IP/hostname where a MPC-HC web interface is listening. Use this to override the config values.")
	aparser.add_argument("--port", "-p",
		help="Specify the PORT on which MPC-HC web interface is listening on. Use this to override the config values.")#, default=13579)
	...
	if args.ui == "text":
		...
		print("You didn't specify a command. Did you mean to run in an interactive mode? If so, run with \"-i text\" or \"-i curses\".")
		sys.exit(1)
